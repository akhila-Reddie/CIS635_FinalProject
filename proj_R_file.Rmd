---
title: "proj-Breast Cancer Detection"
output:
...
Splitting the data into train and test data using dplyr package
```{r}
library(dplyr)
df$id <- 1:nrow(df)
...
SVM model for different kernels
```{r}
library(e1071)
# Linear kernel
svm <- svm(diagnosis ~ ., data = trn, kernel = "linear")
svm_pred <- predict(svm, newdata = tst)
confusionMatrix(svm_pred, as.factor(tst$diagnosis))
```
```{r}
# Radial kernel
svm <- svm(diagnosis ~ ., data = trn, kernel = "radial")
svm_pred <- predict(svm, newdata = tst)
confusionMatrix(svm_pred, tst$diagnosis)
```
```{r}
# Sigmoid kernel
svm <- svm(diagnosis ~ ., data = trn, kernel = "sigmoid")
svm_pred <- predict(svm, newdata = tst)
confusionMatrix(svm_pred, tst$diagnosis)
```
```{r}
# Polynomial kernel
svm <- svm(diagnosis ~ ., data = trn, kernel = "polynomial")
svm_pred <- predict(svm, newdata = tst)
confusionMatrix(svm_pred, tst$diagnosis)
```
